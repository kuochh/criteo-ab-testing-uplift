...
qini = pd.read_csv(os.path.join(DATA_DIR, "qini_curves.csv"))
...
uplift = pd.read_csv(os.path.join(DATA_DIR, "uplift_curves_comparison.csv"))
...
cate_pe_sampled = cate_pe.sample(n=min(5000, len(cate_pe)), random_state=42)
...
pe_late_sampled = pe_late_pos.sample(n=min(5000, len(pe_late_pos)), random_state=42)
